%% Simulación con el termino de difución $\sigma$
%
%%
% Semillas 18,45
rng(35)
% rng('default') % Anulación de las semillas
%% Paramétros
%
a = 0.9;
lambda_0 = 0.9;
...
T = T(1:(end-1));
% hold off
% plot(saltos(1,:),saltos(2,:),'bx')
% hold on
% plot(T(1:end),saltos(2,1:end),'kx')
% hold on
% plot(T(1:end),saltos(3,1:end),'rx')
...
%browniano1 = sim_browniano(maturity,anual,saltos,cantidad);
for i = 1:simus
    inf = saltos(1,i);
    sup = saltos(1,i+1);
    t = linspace(inf,sup,cantidad);
...
nombre = 'simulacion_512.csv';
writetable(G,nombre);
% type simulacion_51.txt;
cla
hold on
% Valores x 55, 69, 72
x=55;
rng(x)
f_trayectoria_55(a,lambda_0,delta,sigma,beta,alpha,rho,0.1,1,'graf_01.csv');
rng(x)
f_trayectoria_55(a,lambda_0,delta,sigma,beta,alpha,rho,1,1,'graf_03.csv');
rng(x)
f_trayectoria_55(a,lambda_0,delta,sigma,beta,alpha,rho,10,1,'graf_03.csv');